...
<style>
  ...
  .tree-node { border: 1px solid #cdd9eb; border-left: 4px solid #1658d4; border-radius: 10px; margin: 14px 0; padding: 14px; background: #f9fbff; } .tree-node h3 { margin: 0 0 8px; font-size: 15px; } .tree-node-grid { display: grid; grid-template-columns: repeat(2, minmax(0,1fr)); gap: 0 12px; } .tree-node-grid .full { grid-column: 1 / -1; } .tree-choice { border-top: 1px solid #dce3ef; margin-top: 10px; padding-top: 10px; display: grid; grid-template-columns: 1.3fr 1.3fr .5fr 1.3fr auto; gap: 8px; align-items: end; } .tree-choice label { margin: 0; } .tree-choice button, .danger { background: #b42318; } .tree-actions { display:flex; flex-wrap:wrap; gap:8px; margin-top: 12px; } .hidden { display: none; }
...
<div class="grid">
  ...
  <section class="card wide"><h2>4. Priebeh hovoru</h2>
    <label>Režim rozhovoru<select id="flowMode"><option value="booking">Predpripravený rezervačný flow</option><option value="tree">Vlastný rozhodovací strom</option></select></label>
    <p class="hint">Rezervačný flow je najrýchlejší pre bežné ambulancie. Stromový režim používa len tebou definované otázky, odpovede a vetvy — žiadny voľný AI chat.</p>
    <div id="bookingFlowNote"><p class="hint">Použije sa výber služby, prípadne člena tímu, preferencia termínu, voľné termíny, meno a záverečné potvrdenie. Texty mimo vlastného úvodu riadi osvedčená rezervačná šablóna.</p></div>
    <div id="treeEditor" class="hidden">
      ...
      <div class="tree-actions"><button id="loadTreeTemplate" class="secondary" type="button">Načítať ukážkový strom</button><button id="addTreeQuestion" type="button">+ Otázka</button><button id="addTreeMessage" class="secondary" type="button">+ Sprievodná veta</button><button id="addTreeEnd" class="secondary" type="button">+ Ukončenie vetvy</button></div>
      <label>Úvodný uzol<select id="treeEntryNode"></select></label>
      <div id="treeNodes"></div>
    </div>
  </section>
  <section class="card wide"><h2>5. Skontroluj a pridaj do Gitu</h2>
...
const treeTemplate = { entryNodeId: 'ako-pomoct', nodes: [{ id: 'ako-pomoct', type: 'question', bridge: '', prompt: 'Dobrý deň, s čím vám dnes môžem pomôcť? Môžete sa objednať na návštevu alebo položiť otázku.', storeAs: 'dovod', confirmSelection: true, choices: [{ id: 'objednanie', label: 'objednať sa na návštevu', voiceAliases: ['objednať sa', 'objednanie', 'chcem termín'], dtmf: '1', nextNodeId: 'typ-navstevy' }, { id: 'otazka', label: 'položiť otázku', voiceAliases: ['mám otázku', 'otázka', 'informácia'], dtmf: '2', nextNodeId: 'prepojenie' }] }, { id: 'typ-navstevy', type: 'question', bridge: 'Ďakujem. Poďme spolu vybrať návštevu, ktorá vám bude najviac vyhovovať.', prompt: 'Vyberte si, prosím, typ návštevy.', storeAs: 'navsteva', confirmSelection: true, choices: [{ id: 'vstupne', label: 'vstupné vyšetrenie', voiceAliases: ['vstupné vyšetrenie', 'prvá návšteva'], dtmf: '1', nextNodeId: 'rezervovane' }, { id: 'kontrola', label: 'kontrolu', voiceAliases: ['kontrola', 'kontrolné vyšetrenie'], dtmf: '2', nextNodeId: 'rezervovane' }] }, { id: 'rezervovane', type: 'end', text: 'Ďakujem. Vaša demo požiadavka na {{navsteva}} je vybavená. Potvrdzujúcu SMS vám posielame hneď po hovore. Ďakujeme a dovidenia.', outcome: 'mock_booking', smsText: 'Ďakujeme. Demo požiadavka na {{navsteva}} je potvrdená.' }, { id: 'prepojenie', type: 'end', text: 'Ďakujem za vašu otázku. Spojím vás s kolegyňou, ktorá vám rada pomôže. Prosím, zostaňte na linke.', outcome: 'handoff' }] };
...
function nodeHtml(node, index) { if (node.type === 'question') { const choices = node.choices.map((choice, choiceIndex) => '<div class="tree-choice"><label>Odpoveď<input data-choice="label" data-node="' + index + '" data-index="' + choiceIndex + '" value="' + esc(choice.label) + '"></label><label>Hlasové synonymá<input data-choice="voiceAliases" data-node="' + index + '" data-index="' + choiceIndex + '" value="' + esc((choice.voiceAliases || []).join(',')) + '"></label><label>Tlačidlo<input data-choice="dtmf" data-node="' + index + '" data-index="' + choiceIndex + '" maxlength="1" value="' + esc(choice.dtmf) + '"></label><label>Pokračovať na<select data-choice="nextNodeId" data-node="' + index + '" data-index="' + choiceIndex + '">' + targets(choice.nextNodeId) + '</select></label><button type="button" data-action="remove-choice" data-node="' + index + '" data-index="' + choiceIndex + '">Odstrániť</button></div>').join(''); return '<article class="tree-node"><h3>Otázka</h3><div class="tree-node-grid"><label>ID uzla<input data-field="id" data-node="' + index + '" value="' + esc(node.id) + '"></label><label>Uložiť odpoveď ako<input data-field="storeAs" data-node="' + index + '" placeholder="napr. navsteva" value="' + esc(node.storeAs) + '"></label><label class="full">Prirodzené premostenie<textarea data-field="bridge" data-node="' + index + '">' + esc(node.bridge) + '</textarea></label><label class="full">Otázka<textarea data-field="prompt" data-node="' + index + '">' + esc(node.prompt) + '</textarea></label><label class="full">HTTPS URL ľudskej nahrávky (nepovinné)<input data-field="audioUrl" data-node="' + index + '" placeholder="https://.../otazka.mp3" value="' + esc(node.audioUrl) + '"></label><label><input type="checkbox" data-field="confirmSelection" data-node="' + index + '"' + (node.confirmSelection ? ' checked' : '') + '> Potvrdiť voľbu áno / nie</label><label>Vlastná potvrdzujúca veta<input data-field="confirmationPrompt" data-node="' + index + '" placeholder="Rozumela som správne, že {{selected}}?" value="' + esc(node.confirmationPrompt) + '"></label></div>' + choices + '<div class="tree-actions"><button type="button" class="secondary" data-action="add-choice" data-node="' + index + '">+ Odpoveď</button><button type="button" class="danger" data-action="remove-node" data-node="' + index + '">Odstrániť otázku</button></div></article>'; } if (node.type === 'message') return '<article class="tree-node"><h3>Sprievodná veta</h3><div class="tree-node-grid"><label>ID uzla<input data-field="id" data-node="' + index + '" value="' + esc(node.id) + '"></label><label>Pokračovať na<select data-field="nextNodeId" data-node="' + index + '">' + targets(node.nextNodeId) + '</select></label><label class="full">Text<textarea data-field="text" data-node="' + index + '">' + esc(node.text) + '</textarea></label><label class="full">HTTPS URL ľudskej nahrávky<input data-field="audioUrl" data-node="' + index + '" value="' + esc(node.audioUrl) + '"></label></div><div class="tree-actions"><button type="button" class="danger" data-action="remove-node" data-node="' + index + '">Odstrániť vetu</button></div></article>'; return '<article class="tree-node"><h3>Ukončenie vetvy</h3><div class="tree-node-grid"><label>ID uzla<input data-field="id" data-node="' + index + '" value="' + esc(node.id) + '"></label><label>Výsledok<select data-field="outcome" data-node="' + index + '"><option value="complete"' + (node.outcome === 'complete' ? ' selected' : '') + '>Ukončiť hovor</option><option value="handoff"' + (node.outcome === 'handoff' ? ' selected' : '') + '>Prepojenie / človek</option><option value="mock_booking"' + (node.outcome === 'mock_booking' ? ' selected' : '') + '>Demo rezervácia + SMS</option></select></label><label class="full">Záverečná veta<textarea data-field="text" data-node="' + index + '">' + esc(node.text) + '</textarea></label><label class="full">SMS text pri demo rezervácii<input data-field="smsText" data-node="' + index + '" value="' + esc(node.smsText) + '"></label><label class="full">HTTPS URL ľudskej nahrávky<input data-field="audioUrl" data-node="' + index + '" value="' + esc(node.audioUrl) + '"></label></div><div class="tree-actions"><button type="button" class="danger" data-action="remove-node" data-node="' + index + '">Odstrániť ukončenie</button></div></article>'; }
function updateField(event) { const element = event.target; if (element.dataset.field === 'id' && event.type === 'input') return; const node = tree.nodes[Number(element.dataset.node)]; const oldId = node.id; node[element.dataset.field] = element.type === 'checkbox' ? element.checked : element.value; if (element.dataset.field === 'id' && oldId === tree.entryNodeId) { tree.entryNodeId = node.id; renderTree(); } }
function updateChoice(event) { const element = event.target; const choice = tree.nodes[Number(element.dataset.node)].choices[Number(element.dataset.index)]; choice[element.dataset.choice] = element.dataset.choice === 'voiceAliases' ? element.value.split(',').map((item) => item.trim()).filter(Boolean) : element.value; }
function treeAction(event) { const element = event.currentTarget; const node = tree.nodes[Number(element.dataset.node)]; if (element.dataset.action === 'add-choice') node.choices.push({ id: 'moznost-' + (node.choices.length + 1), label: '', voiceAliases: [], dtmf: String(node.choices.length + 1), nextNodeId: '' }); if (element.dataset.action === 'remove-choice') node.choices.splice(Number(element.dataset.index), 1); if (element.dataset.action === 'remove-node') tree.nodes.splice(Number(element.dataset.node), 1); if (!tree.nodes.some((item) => item.id === tree.entryNodeId)) tree.entryNodeId = tree.nodes[0] ? tree.nodes[0].id : ''; renderTree(); }
function renderTree() { $('treeEntryNode').innerHTML = tree.nodes.map((node) => '<option value="' + esc(node.id) + '"' + (node.id === tree.entryNodeId ? ' selected' : '') + '>' + esc(node.id) + '</option>').join(''); $('treeNodes').innerHTML = tree.nodes.map(nodeHtml).join(''); document.querySelectorAll('[data-field]').forEach((item) => { item.addEventListener('input', updateField); item.addEventListener('change', updateField); }); document.querySelectorAll('[data-choice]').forEach((item) => { item.addEventListener('input', updateChoice); item.addEventListener('change', updateChoice); }); document.querySelectorAll('[data-action]').forEach((item) => item.addEventListener('click', treeAction)); }
function addTreeNode(type) { const n = tree.nodes.length + 1; if (type === 'question') tree.nodes.push({ id: 'otazka-' + n, type: 'question', bridge: '', prompt: '', storeAs: '', confirmSelection: true, choices: [{ id: 'moznost-1', label: '', voiceAliases: [], dtmf: '1', nextNodeId: '' }] }); if (type === 'message') tree.nodes.push({ id: 'veta-' + n, type: 'message', text: '', nextNodeId: '' }); if (type === 'end') tree.nodes.push({ id: 'koniec-' + n, type: 'end', text: '', outcome: 'complete' }); if (!tree.entryNodeId) tree.entryNodeId = tree.nodes[0].id; renderTree(); }
...
$('flowMode').onchange = (event) => setFlowMode(event.target.value); $('treeEntryNode').onchange = (event) => { tree.entryNodeId = event.target.value; }; $('loadTreeTemplate').onclick = () => { tree = JSON.parse(JSON.stringify(treeTemplate)); setFlowMode('tree'); renderTree(); }; $('addTreeQuestion').onclick = () => addTreeNode('question'); $('addTreeMessage').onclick = () => addTreeNode('message'); $('addTreeEnd').onclick = () => addTreeNode('end');
