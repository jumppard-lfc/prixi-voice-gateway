...
async function save() { const data = config(); const response = await fetch('/admin/voice-bot-builder/save', { method: 'POST', headers: { 'content-type': 'application/json', 'authorization': 'Bearer ' + token }, body: JSON.stringify(data) }); if (!response.ok) { const result = await response.json().catch(() => ({})); $('feedback').innerHTML = '<span class="error">' + (result.message || 'Konfiguráciu sa nepodarilo uložiť.') + '</span>'; return; } const result = await response.json(); const routingHint = result.inboundTwilioNumbers.length ? '<br><span class="hint">Pre dedikované Twilio číslo môžeš alternatívne použiť spoločný webhook <code>/voice/incoming</code>; router si bota vyberie podľa čísla To.</span>' : ''; $('feedback').innerHTML = '<span class="ok">Demo bot je uložený. Nastav vo Twilio tento Voice webhook:</span><br><code>' + result.webhookPath + '</code>' + routingHint + '<br><span class="hint">Pre reálnu SMS zapni DEMO_BOOKING_SMS_ENABLED=true a nakonfiguruj BulkGate.</span>'; $('output').textContent = JSON.stringify(result.config, null, 2); }
